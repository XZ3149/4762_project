...
def mat_to_csv():
    ...
    """
    matrix_dir = "C:\\Users\\we609\\Desktop\\cs 4762\\project\\10K\\filtered_feature_bc_matrix"
    mat = scipy.io.mmread(os.path.join(matrix_dir, "matrix.mtx.gz"))
    cell_number = mat.shape[1]
    threshold = 0.2
...
def generate_k_nearests_dict(df):
    """this is the function that can generate a dict that contain values 
    this will greatly imporved k_nearests_neighbor computation speed
    """
    dic = {}
    dimension = len(list(df.loc[1]))
...
def k_nearest_neighbors_smooth_account_initial(peaks_matrix, k_nearest_matrix, k = 30, contribution = 0.7):
    """
    this is the function using k_nearest_matrix to smooth peaks_matrix
    peaks_maxtrix: [P x N]
    k_nearest_matrix: [N x N] 
    ...
    """
    result = ((peaks_matrix @ k_nearest_matrix) * 1/k * contribution) + (1-contribution) * peaks_matrix 
    return result
...
def sample_split(df_gene_expression, df_peaks, N, random_seed = 123):
    """this is the function that split the data into 80% training, 10% validation, 
    and 10% testing
...
def ridge_regression_gd(X, y, lamb, distants, learning_rate = 0.001, iterations = 200): 
    ...
    for k in range(iterations): # in practice we would use an appropriate "stopping criteria"
        ...
        losses.append(loss.item())
        grad = - (X.transpose(0,1) @ err)/X.shape[0] +  lamb * beta * distants
        previous_loss = loss
        beta -= learning_rate * grad
    return beta
